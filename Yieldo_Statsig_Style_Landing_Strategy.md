# yieldo 랜딩페이지 전략 가이드 (Statsig 스타일 적용)

## 0. 목적

이 문서는 Statsig 스타일의 B2B SaaS 랜딩페이지 구조를 참고하되, yieldo의 정체성을 유지하면서  
...
**yieldo turns market, experiment, UA, and financial data into one investment decision layer for game operators.**

### Korean Hero
**데이터 근거로 게임 포트폴리오를 운영하세요.**  
**또 하나의 대시보드가 아닙니다. 투자 판단을 위한 의사결정 레이어입니다.**  
**yieldo는 시장, 실험, UA, 재무 데이터를 하나로 연결해 게임 운영자를 위한 투자 의사결정 기반을 제공합니다.**
...
yieldo도 이 방식은 맞지만, Statsig와 달리 제품 구조의 맨 앞에 **investment decision**이 와야 한다.
...
## 3. yieldo에 맞는 랜딩페이지 핵심 전략

Statsig처럼 만들되, yieldo는 반드시 아래 3가지를 사용자에게 첫 화면 근처에서 이해시켜야 한다.
...
즉 yieldo 랜딩페이지는 “기능 소개 페이지”가 아니라  
...
### 마지막 문장
**yieldo connects all four into one investment decision.**

### 이미지
- 회색 silo cards
- 화살표가 끊긴 diagram
- 중앙에 yieldo가 연결하는 before/after graphic

---

## Section 3 — What yieldo actually answers
### 목표
기존 SaaS들이 답하지 못하는 질문을 yieldo가 답한다고 보여줌
...
Statsig의 “Integrated products” 섹션을 yieldo식으로 변형한 핵심 섹션

### 섹션 헤드라인
**There’s a smarter way to run a game portfolio.**

### 설명
yieldo gives operators a unified decision layer across market intelligence, UA, experiments, and financials.
...
### 카피 예시
yieldo gives game operators one investment signal, the evidence behind it, and the next action to take.
...
Statsig가 제품군을 각각 큰 이미지로 소개하는 방식처럼, yieldo는 차트를 각각 “기능 이야기”로 소개해야 한다.
...
### 설명
yieldo explains why payback moved, what changed in retention or CPI, and how much those changes matter.
...
### 설명
yieldo translates experiment results into ΔLTV, payback movement, and capital efficiency — so winning tests become funding decisions.
...
- yieldo tells you whether to invest more
...
### 보조 문구
See how yieldo turns game data into investment decisions.
...
yieldo는 여기에 한 가지를 더해야 한다.

Statsig가 “product development platform”이라면,  
yieldo는 반드시 **investment decision layer for game operators**로 읽혀야 한다.
