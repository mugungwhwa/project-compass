# yieldo — Technical Stack & Requirements Document

## Signal-to-Yield Operating Terminal for Mobile Gaming
...
yieldo의 마켓 데이터 전략은 **앱 인텔리전스 플랫폼 의존이 아닌, 자체 데이터 자산 구축**입니다.
...
기존 번역 파이프라인은 "ATE → ΔLTV per user"에서 멈춘다. yieldo의 차별화는 **"이 variant를 5% → 25% → 50% → 100%로 확장하면 포트폴리오 전체 LTV가 어떻게 파급되는가"**를 단계별로 예측하는 것이다.
...
**적응형 롤아웃 루프 (optional):**
Contextual Bandit (mabwiser)로 variant별 트래픽 비율을 자동 조정 — 투자 가치가 크레디블 인터벌 하한에서도 양수면 트래픽 확대, 음수로 드리프트하면 축소/롤백. 이것이 yieldo가 "Decision OS"로 불리는 이유다 (단순 A/B 리포팅이 아닌 **자본 배분 자동 조정**).
...
yieldo는 자체 데이터를 생산하는 도구가 아니라, **기존 도구들의 출력을 투자 결정으로 번역하는 레이어**입니다.
...
| **데이터 형식** | JSON |
| **필요 플랜** | 고객사가 AppsFlyer 계약 필요 (yieldo 비용 아님) |

```typescript
// yieldo에서 AppsFlyer 데이터를 가져오는 흐름
...
// yieldo가 수신하여 투자 결정에 사용하는 핵심 필드
...
│                         ▼                                │
│    ┌─────────────────────────────────────────┐           │
│    │  yieldo Investment Engine               │           │
│    │  MMP CPI + yieldo Retention Prediction  │           │
...
```
MMP 데이터 (실제 관측)              yieldo 예측 엔진                투자 결정
━━━━━━━━━━━━━━━━━━━━              ━━━━━━━━━━━━━━━━━              ━━━━━━━━━━
...
interface ExperimentInvestmentTranslation {
  ...
  // yieldo가 계산하는 투자 가치 (P10/P50/P90)
...
Firebase 연동 시 데이터 흐름:
  Firebase A/B → BigQuery Export → yieldo Python 서비스
...
```
┌─────────────────────────────────────────────────────────────┐
│            yieldo Rollout Orchestration Loop                │
│                                                             │
│  ┌──────────┐    ┌───────────────┐    ┌────────────────┐   │
...
| ATE 방향 | 투자 가치 (P50) | P(ROI > 0) | yieldo 결정 | 롤아웃 액션 |
|---|---|---|---|---|
...
Day 0:  실험 Ship 결정 (ATE = +3.7pp D7 리텐션, P(ROI>0) = 82%)
        └─ yieldo 권장: "점진 확대" — 먼저 10%에 배포

Day 3:  10% 롤아웃 관측
        ├─ 실제 리텐션 변화: +3.2pp (예측 범위 내)
        ├─ 매출 영향: +$12K (예측 $15K의 80%)
        └─ yieldo: "관측값이 P10-P90 밴드 내 → 25%로 확대 권장"

Day 7:  25% 롤아웃 관측
        ├─ 실제 리텐션 변화: +4.1pp (예측보다 좋음!)
        ├─ P(ROI>0) 업데이트: 82% → 91%
        └─ yieldo: "신뢰도 상승 → 50%로 확대 권장"

Day 14: 50% 롤아웃 관측
        ├─ 실제 ΔLTV 관측: $0.85/user
        ├─ 연간 매출 증분 예측: $510K (P50)
        └─ yieldo: "충분한 확신 → 100% 전체 배포 권장"
...
이것이 yieldo의 **핵심 지적 재산**입니다. 어떤 기존 도구도 이 번역을 수행하지 않습니다.
...
│  INPUT (3개 소스에서 수집)                                        │
│  ┌──────────────┐ ┌──────────────┐ ┌─────────────────────────┐  │
│  │ MMP Data      │ │ A/B Results  │ │ yieldo Retention Model │  │
│  │ • CPI         │ │ • ATE        │ │ • R(t) curve            │  │
...
#### 전제: yieldo는 A/B 테스트 SDK가 아니다

CLAUDE.md Section 1.3에 명시된 대로 yieldo는 Statsig, Firebase를 대체하지 않습니다.
하지만 현실적으로 **A/B 플랫폼 없이 운영하는 소규모 게임사**가 많습니다.
이 고객들도 "실험 → 투자 가치" 번역의 혜택을 받을 수 있도록, 최소한의 약식 지원을 제공합니다.

> 핵심 원칙: **yieldo가 실험을 실행하지 않는다. 실험 결과를 입력받아 투자 가치로 번역한다.**

#### 3가지 약식 모드

##### Mode 1: 수동 실험 등록 (Manual Entry)

가장 가벼운 방식. 고객이 자체적으로 실험한 결과를 yieldo에 직접 입력합니다.
...
**필요 기술:** NumPyro (이미 있음), yieldo UI 폼 (React Hook Form)
...
yieldo는 이를 **결정 신뢰도(decision confidence)에 반영**합니다:
...
1. 고객이 A/B 플랫폼 없음
   → yieldo 약식 모드 (수동 등록 + Pre/Post) 제공
   → 투자 가치 번역 결과를 경험하게 함
...
- [ ] **MMP 데이터 → Revenue Chain 연결** — CPI + Installs → yieldo 리텐션 예측 → LTV/Payback
