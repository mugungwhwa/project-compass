...
**Status**: 시연용 — connections PR-A + PR-B main 안착 후
...
## 4. 재무 직접 입력 시연 — 핵심 시퀀스 (40초)

이 부분이 narrative의 가장 강력한 단계 — input → 저장 → 영구화의 흐름.
...
6. 카드 다시 클릭 → 폼이 저장된 값으로 pre-fill

> "Visible.vc 모델 — 3분 안에 5개 핵심 지표만 입력. localStorage에 저장돼 새로고침에도 살아남습니다. 정식 버전은 Supabase에 영구 저장 + RLS로 멀티테넌트 격리합니다."

핵심 단어: **Visible.vc**, **3분**, **localStorage prototype → Supabase 정식**.
...
> "이 데모는 4개 사일로 통합 진입점입니다. 다음 단계는 (1) 실제 OAuth connector 추가, (2) 재무 입력값이 Module 1 Executive Overview burn tolerance 카드로 흐르게 wiring, (3) 시장 인텔리전스 Bayesian prior 자동 갱신. 6주 안에 paying customer 1팀 pilot이 목표입니다."
...
git revert 안전 zone:
- PR-B만 revert: `git revert -m 1 c0567dc`
- PR-A까지 revert: `git revert -m 1 7aacf83 c0567dc` (역순)

---

## References

- PR #13 (PR-A): connections demo-pack
- PR #14 (PR-B): financial manual input
...
- Memory: `project_connections_demo_pack_shipped.md`
