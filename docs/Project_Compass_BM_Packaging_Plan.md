# Project Compass — Business Model & Packaging Plan

> **목적**: Compass의 수익화 전략, 티어별 패키징, 단가 책정, 그리고 Phase별 진화 로드맵을 하나의 문서로 통합
>
...
> Compass는 **Consulting → SaaS → Enterprise** 3단계 수익 진화를 따른다.
...
Compass가 답하는 질문은 "이 게임에 추가 투자해야 하나?"이다.
...
| **성장 연동** | 고객이 성공할수록 Compass 매출도 성장 (타이틀 수 × 깊이) |
...
**핵심 전략**:
- 모든 컨설팅은 Compass 대시보드를 보조 도구로 사용 → 제품 가치 자연 체험
...
| **Strategic Advisory** | C-level 의사결정 자문 (Compass 데이터 기반) | 프리미엄 리테이너 |
...
**타겟**: 소규모 스튜디오, 단일 타이틀 운영사, Compass 첫 도입
...
**핵심 차별화**: Module 3+4 (개입→투자 번역)가 Compass의 핵심 가치이며, Growth부터 열림
...
| **Advisory** | 분기별 C-level 투자 전략 리뷰 (Compass 데이터 기반) |
...
| 시나리오 | Compass 없이 | Compass 있으면 | 가치 차이 |
|---|---|---|---|
...
**Compass 포지셔닝**: 이 도구들 위에 앉는 "번역 레이어" → 이들의 합산 비용 대비 30-50% 수준이 적정
...
- URL: `demo.compassgames.io` (또는 `/demo` 경로)
- 로그인 불필요, 모든 모듈 탐색 가능
...
- `Project_Compass_Business_Plan.md` §9 — 상세 단가, §14 — 재무 예측
- `docs/Project_Compass_B2B_Readiness_Blueprint.md` §2 — Gap Matrix, §4 — 단계 목표
- `Project_Compass_Tech_Stack.md` §7.5 — 인프라 비용 구조
