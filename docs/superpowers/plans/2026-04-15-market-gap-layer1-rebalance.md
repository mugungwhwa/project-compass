...
**Spec 출처:**
- `docs/superpowers/specs/2026-04-15-compass-positioning-language-layering-design.md` §3-§4
...
**신규 (Create):**
- `compass/src/shared/lib/market-signal.ts` — 판정 신호 helper (pure function)
- `compass/src/shared/lib/market-signal.examples.ts` — usage example (테스트 러너 없으므로 assertion script 대체)

**수정 (Modify):**
- `compass/src/shared/config/chart-colors.ts` — `MARKET_GAP_PROOF_COLORS` 추가 (기존 `PRIOR_POSTERIOR_COLORS`는 L2 methodology 차후 작업용으로 유지)
- `compass/src/shared/i18n/dictionary.ts` — `market.proof.*` + `market.signal.*` 네임스페이스 신규 추가 (기존 `market.priorPosterior`/`market.bayesian`/`market.priorLabel`/`market.posteriorLabel`/`info.priorPosterior`는 L2 차후용 유지)
- `compass/src/shared/lib/index.ts` — 신규 helper re-export
- `compass/src/widgets/charts/ui/prior-posterior-chart.tsx` — 전면 rewrite

**건드리지 않음:**
- `compass/src/shared/api/mock-data.ts` (mockPriorPosterior 그대로 — 데이터 구조 변경 없음)
- `compass/src/app/(dashboard)/dashboard/market-gap/page.tsx` (컴포넌트 props 시그니처 변경 없음)
- `compass/src/shared/ui/info-hint.tsx` (어제 수정 그대로 사용)
...
Run: `grep -n "PRIOR_POSTERIOR_COLORS" /Users/mike/Downloads/Compass/compass/src/shared/config/chart-colors.ts`
...
Run: `cd /Users/mike/Downloads/Compass/compass && npx tsc --noEmit 2>&1 | grep -v "decision-surface" | head -20`
...
git add compass/src/shared/config/chart-colors.ts
...
Run: `grep -n "market.priorPosterior\|market.posteriorLabel" /Users/mike/Downloads/Compass/compass/src/shared/i18n/dictionary.ts`
...
Run: `grep -c "market.proof\." /Users/mike/Downloads/Compass/compass/src/shared/i18n/dictionary.ts`
...
**Files:**
- Create: `compass/src/shared/lib/market-signal.ts`
- Create: `compass/src/shared/lib/market-signal.examples.ts`
- Modify: `compass/src/shared/lib/index.ts`

- [ ] **Step 1: shared/lib/index.ts 현황 확인**

Run: `cat /Users/mike/Downloads/Compass/compass/src/shared/lib/index.ts`
Expected: `export * from "./utils"` 같은 줄들. 없으면 파일 자체 미존재 가능 — Run: `ls /Users/mike/Downloads/Compass/compass/src/shared/lib/` 로 확인

- [ ] **Step 2: `market-signal.ts` 작성**

파일 생성: `/Users/mike/Downloads/Compass/compass/src/shared/lib/market-signal.ts`
...
파일 생성: `/Users/mike/Downloads/Compass/compass/src/shared/lib/market-signal.examples.ts`
...
Run: `cd /Users/mike/Downloads/Compass/compass && npx tsx src/shared/lib/market-signal.examples.ts`
...
```bash
cd /Users/mike/Downloads/Compass
git add compass/src/shared/lib/market-signal.ts compass/src/shared/lib/market-signal.examples.ts compass/src/shared/lib/index.ts compass/package.json compass/package-lock.json
...
Run: `cat /Users/mike/Downloads/Compass/compass/src/widgets/charts/ui/prior-posterior-chart.tsx`
전체 파일 이해 후 진행.
...
Run: `cd /Users/mike/Downloads/Compass/compass && npm run dev` (이미 dev 서버 돌고 있으면 skip — 기존 세션 확인)
