...
**Page**: `yieldo/src/app/(dashboard)/dashboard/market-gap/page.tsx`
...
yieldo의 primary user는 data analyst가 아닌 business operator (CLAUDE.md §11.2). 치환성+순환성이 통계 용어 없이도 직관적으로 읽히는가?
...
- **장점**: 진짜 베이지안 process를 보여줌, "이건 진짜 추론이다" 증명력 강함, yieldo 차별점 극대화
...
## 10. 후속 업데이트 (2026-04-15 재조정, brainstorming 결과 반영)

**중요**: 이 섹션은 2026-04-14 초안에서 "yieldo = Alpha 측정 인프라"라는 top-line 포지션을 제안했으나, 2026-04-15 brainstorming 세션에서 외부 검증 없이 단독 합의된 리스크를 식별하고 재조정함. 상세는 `2026-04-15-yieldo-positioning-language-layering-design.md`.

**최종 결정**:

- **Top-line 포지션 = Decision OS** (CLAUDE.md §1 원본 유지, 변경 없음)
- **Alpha는 L2 signature mechanism** — Methodology 패널 / 투자자 pitch 맥락에서만 등장
- **Alpha 단어 자체는 잠정 사용, 외부 검증 대기** (operator 3+ / 투자자 2+ 인터뷰 후 확정)

| 의미 레이어 | 통계 | L0/L1 (Operator) | L2 (Methodology) |
|---|---|---|---|
| 시장 기대 | Prior | 장르 기대치 | Market Consensus / Beta |
| 우리 측정 | Posterior | 우리 실적 | Our Actuals / Realized |
| 격차 | Gap | 우리 우월/부족 ±Z% | Alpha (잠정) |
| 지속성 | Cyclic Update | (L1 미노출) | Alpha Persistence |
| 판정 신호 | — | Invest/Hold/Reduce | — |

Market Gap 페이지의 L1은 "장르 기대치 vs 우리 실적" 언어로 통일, Alpha 단어 미노출. L2 methodology 패널에서만 Alpha 용어 허용. 상세 spec: `2026-04-14-market-gap-bayesian-p3-implementation.md` §3.1 (재조정판), `2026-04-15-yieldo-positioning-language-layering-design.md` §4.
