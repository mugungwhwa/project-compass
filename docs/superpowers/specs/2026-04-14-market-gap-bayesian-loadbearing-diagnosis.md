...
## 10. 후속 업데이트 (2026-04-14 추가)

진단 후 Mike 추가 질문 — "시장 격차를 비즈니스 용어로 풀 수 있나?" — 에서 나온 핵심 재프레임:

**Market Gap = Alpha 측정** (hedge-fund 스타일 edge 시각화)

| 통계 | Alpha 모델 | operator 언어 |
|---|---|---|
| Prior | Benchmark / Consensus / Beta | 시장 컨센서스 |
| Posterior | Actuals / Realized Return | 우리 실적 |
| Gap | **Alpha / Outperformance** | Alpha (+/−) |
| Cyclic Update | Alpha Persistence | Alpha 지속성 검증 |

이 프레임으로 **Compass = "Alpha 측정 인프라"** 라는 카테고리 포지션(CLAUDE.md §12)이 Market Gap 페이지에서 직접 증명됨. Module 1 HeroVerdict의 Invest/Hold/Reduce 판정이 Alpha 부호와 1:1 대응되어 페이지가 자체 정당화.

상세 구현은 `2026-04-14-market-gap-bayesian-p3-implementation.md` §2 "비즈니스 언어 오버레이 — Alpha/Beta 프레임" 참조.
