...
**2026-04-15 재조정 (brainstorming 결과)**: Alpha를 top-line positioning으로 밀었던 어제 결정 철회. Alpha는 **yieldo Decision OS의 signature mechanism**이며 **L2(Methodology / Investor Pitch) 레이어 전용**. yieldo의 top-line은 CLAUDE.md §1 원본 "Signal-to-Yield Operating Terminal for mobile gaming" 그대로 유지.

상세 근거: `2026-04-15-yieldo-positioning-language-layering-design.md`

이 프레임은 P3 Layer 1(operator UI)에는 노출하지 않고, Layer 2(methodology 토글 내부)에서만 작동. Layer 1은 의사결정 언어(Invest/Hold/Reduce 신호)로 통일.

### 2.1 3-layer 용어 매핑

| 통계 레이어 (원전) | 금융 Alpha 모델 | yieldo operator 언어 (Layer 1) | 의사결정 의미 |
|---|---|---|---|
...
- **yieldo → Alpha 측정 + 지속성 검증 인프라** ← 이 포지션이 P3에서 드러남

Market Gap 페이지는 yieldo의 "alpha 측정 인프라" 정체성을 **한 화면으로 증명**하는 역할.
...
**L2 methodology 패널**에서만 Alpha · Posterior · Prior · Bayesian 용어 허용. 관련 세부: `2026-04-15-yieldo-positioning-language-layering-design.md` §4.
...
│  ─────────────────────────────────────                   │
│  📚 yieldo는 매 cohort 도착 시 Bayesian posterior       │
